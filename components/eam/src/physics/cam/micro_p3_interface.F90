...
!! pbuf 
   integer :: &
      ...
      dei_idx,            &
!<shanyp 03112023
      cldfsnow_idx,       &  ! add the snow cloud fraction.
      des_idx,       &  ! add the snow cloud fraction.
      icswp_idx,     &
!      icsnc2,       &  !
!      icsmrst2,       &  !
!      icinc2_idx,    &  ! add the snow cloud fraction.
      icimrst2_idx,  &  !
!shanyp 03112023>
      rate1_cw2pr_st_idx, &
      mu_idx,             &
...
!<shanyp 03112023
!   cldfsnow_idx = pbuf_get_index('CLDFSNOW')
   call pbuf_add_field('CLDFSNOW ',  'global',dtype_r8,(/pcols,pver,dyn_time_lvls/), cldfsnow_idx)
   call pbuf_add_field('ICSWP',  'global',dtype_r8,(/pcols,pver/), icswp_idx)
   call pbuf_add_field('DES',  'global',dtype_r8,(/pcols,pver/), des_idx)
!   call pbuf_add_field('ICINC2',  'global',dtype_r8,(/pcols,pver/), icinc2_idx)
   call pbuf_add_field('ICIMRST2',  'global',dtype_r8,(/pcols,pver/), icimrst2_idx)
!shanyp 03112023>
...
!<shanyp 02172023
   call addfld('P3_qiberg', (/ 'lev' /), 'A', 'kg/kg/s', 'P3 Tendency for WBF')
!shanyp 02172023>
...
!shanyp 02172023>
  ...
  subroutine micro_p3_tend(state, ptend, dtime, pbuf)
    ...
    use time_manager,   only: get_nstep
!<shanyp 03102023
    use micro_p3,       only: p3_main, bfb_gamma,bfb_cbrt
!shanyp 03102023>
...
!<shanyp 03082023
    real(rtype) :: avgrhoi,prhoi,crhoi
    real(rtype) :: icsmrst2(pcols,pver)
    real(rtype), pointer :: cldfsnow(:,:),icswp(:,:),des(:,:),icimrst2(:,:)
    icsmrst2(:,:)=0._rtype
    avgrhoi=0._rtype ! average density of entire ice
    prhoi=900._rtype ! kg/m3, density of primary ice (not generated by riming)
    crhoi=400._rtype ! kg/m3, criteria ice density to identify the snow crystals
!shanyp 03082023>
...
!<shanyp 03112023
! Get index, get pbuf field, and initialize the field. CLDFSNOW, ICSWP, and, DES
! are passed to subroutine radiation_tend (physics/rrtmgp/radiation.F90). The
! icimrst2 is passed to subroutine conv_water_4rad in the
! cloud_diagnostics.F90
    ...
    call pbuf_get_field(pbuf, cldfsnow_idx, cldfsnow, start=(/1,1,itim_old/), kount=(/psetcols,pver,1/) )
...
!<shanyp 03112023
! Here, we use the stratiform cloud fraction as the cldf for stratiform snow.
! This differs from MG2 setup that uses total cldf (st+cu) for stratifomr snow.
! In the CLUBB+MG2+RRTMGP: ast+deepcu-->cloud_frac-->cld-->cldfsnow
! In the CLUBB+P3+RRTMGP: ast-->cldfsnow
    cldfsnow(:ncol,top_lev:pver)=ast(:ncol,top_lev:pver) 
!shanyp 03112023> 
    ! INITIALIZE PTEND
    !==============
...
!shanyp 03082023
   do k = top_lev, pver
      do icol = 1, ncol
         if (ice(icol,k).gt.0) then !((icimrst(icol,k).gt.0).and.(icinc(icol,k).gt.0)) then
! Calculate the density of entire ice: avgrhoi=qi/((qi-qm)/rhoi+rimvol), where
! (qi-qm)/rhoi is the primary ice volume. 
!          avgrhoi=ice(icol,k)/(max((ice(icol,k)-qm(icol,k))/prhoi,0._rtype)+rimvol(icol,k))
! if rimed ice fraction (qm/qi)<0.1  AND density of entire ice (avgrhoi) < 400 kg/m3, all ice particles are treated as snow, otherwise they are ice. 
          if((qm(icol,k)/ice(icol,k).lt.0.1).and.(rho_qi(icol,k).lt.crhoi)) then 
           icsmrst2(icol,k) = icimrst(icol,k)
           icswp(icol,k) = icsmrst2(icol,k)*state%pdel(icol,k)/gravit ! calculate snow water path (in-cloud value)
           des(icol,k) = dei(icol,k) ! snow effective diameter (um)
! No ice in this grid. So, in-cloud ice mass mixing ratio (icimrst2) and ice
! effective radius (rei) are zero.
           icimrst2(icol,k) = 0._rtype
           rei(icol,k) = 0._rtype
          else
! No snow in this grid. So, snow mass, water path and effective diameter are
! zero.
           icsmrst2(icol,k) = 0._rtype
           icswp(icol,k) = 0._rtype
           des(icol,k) = 0._rtype
! All in-cloud ice mass mixing ratio is passed to icimrst2. Change to rei is not
! needed. 
           icimrst2(icol,k) = icimrst(icol,k)
          end if
         else
! For clear sky cases, all cloud properties are zero. 
          icimrst2(icol,k) = 0._rtype
          icsmrst2(icol,k) = 0._rtype
          icswp(icol,k) = 0._rtype
          des(icol,k) = 0._rtype
  ...
  ! write(iulog, *) 'LLILY1',count(icswp.gt.0._rtype),count(icimrst2.gt.0._rtype),real(count(icswp.gt.0._rtype))/real(count(icimrst2.gt.0._rtype))
! shanyp 03082023>
...
!   call outfld('P3_qcmul',               tend_out(:,:,34), pcols, lchnk) ! Not actually used, so not actually recorded.  Commented out here for continuity of the array.
   call outfld('P3_qiberg',               tend_out(:,:,34), pcols, lchnk)
!shanyp 02172023>
