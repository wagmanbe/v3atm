...
!kzm++
#if (defined MODAL_AERO_4MODE_MOM || defined MODAL_AERO_4MODE_SOA_MOM ||  defined MODAL_AERO_5MODE)
...
!kzm++
         ...
         if (as_bc > 0._r8) then
            ...
            bc_num = as_bc/(as_so4+as_bc+as_pom+as_soa+as_ss+as_du+as_mom+&
                            as_nh4+as_no3+as_ca+as_co3+as_cl)  &
                     * aer(ii,kk,num_accum)*1.0e-6_r8 ! #/cm^3
!kzm++,  QZR++ and at similar instances to ADD MODAL_AERO_4MODE_SOA_MOM
#elif ( defined MODAL_AERO_4MODE_MOM || defined MODAL_AERO_4MODE_SOA_MOM || defined MODAL_AERO_5MODE)
            bc_num = as_bc/(as_so4+as_bc+as_pom+as_soa+as_ss+as_du+as_mom)  &
                     * aer(ii,kk,num_accum)*1.0e-6_r8 ! #/cm^3
#else
...
!QZR++
      ...
      if (dmc > 0._r8 ) then
! QZR ++ || defined MODAL_AERO_4MODE_SOA_MOM
#if ( ( defined MODAL_AERO_4MODE_MOM || defined MODAL_AERO_4MODE_SOA_MOM ) && ( defined RAIN_EVAP_TO_COARSE_AERO ) && ( defined MOSAIC_SPECIES ) )
         dst3_num = (dmc+camc+co3mc)/(ssmc+dmc+bcmc+pommc+soamc+mommc+nh4mc+no3mc+camc+co3mc+clmc) * aer(ii,kk,num_coarse)*1.0e-6_r8 ! #/cm^3
!kzm++
#elif ( (defined MODAL_AERO_4MODE_MOM || defined MODAL_AERO_4MODE_SOA_MOM ||  defined MODAL_AERO_5MODE) && defined RAIN_EVAP_TO_COARSE_AERO )
