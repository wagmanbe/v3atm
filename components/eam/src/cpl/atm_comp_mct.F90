...
! Added by U-MICH team on Dec.15, 2019 -->
    ...
    real(r8) :: pi
    real(r8) :: emis0(pcols*(endchunk-begchunk+1),nlwbands)
...
! This change is to compute surface skin temperature and pass it to cam_in
    ...
    !if (.false.) then
       call read_surface_emis(sizebuf,lats2,lons2,mon,emis0(1:sizebuf,:), &
                              water_emis, ice_emis, desert_emis, grass_emis)
         ...
         do c=begchunk, endchunk
           ...
           do i=1,ncols
             ...
             ! added for restart, xiuhong, Apr 10, 2020-->
             x2a_a%rattr(index_x2a_Sl_ts_atm, sizebuf) = cam_in(c)%ts_atm(i)
             do j=1,nlwbands
                x2a_a%rattr(index_x2a_Sl_srf_emis_spec(j),sizebuf) = &
                        cam_in(c)%srf_emis_spec(i,j)
             end do
             ! end add, xiuhong, Apr 10, 2020.
...
! U-MICH team added the following subroutines/functions -->  
  ...
  subroutine read_surface_emis(ncols,lats2,lons2,mn,surface_emis, water_emis, ice_emis, desert_emis, grass_emis)
   ...
   !  This subroutine is to read surface emissivity from dataset
       ...
       real(r8),intent(out) :: water_emis(nlwbands), ice_emis(nlwbands), desert_emis(nlwbands), grass_emis(nlwbands)
